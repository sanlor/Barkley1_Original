...
for (ix = 0; ix < w; ix += 20)
    for (iy = 0; iy < h; iy += 20)
        ...
        p_array[i,9] = 1;  // alpha
        p_array[i,10] = 0; // speed
        p_array[i,11] = 0; // direction
        ...
        p_array[i,16] = random(1/2-1/20) + 1/20; // Extra var
        p_array[i,17] = 1; // Extra var II
...
while (p_alive > 0)
    ...
    for (i = 0; i < p_count; i += 1)
        ...
        draw_surface_part_ext(p_array[i,14],p_array[i,3],p_array[i,4],p_array[i,5],p_array[i,6],p_array[i,1] + p_array[i,5]/2*(1-p_array[i,7]),p_array[i,2] + p_array[i,6]/2*(1-p_array[i,8]),p_array[i,7],p_array[i,8],c_white,p_array[i,9]);
        ...
        p_array[i,17] -= p_array[i,16];
        if (p_array[i,17] <= 0) && (p_array[i,0] > 0) {p_array[i,0] = 0; p_alive -= 1;}
        }
